...
export const createCartProductElement = ({ id, title, price, pictures }) => {
  ...
  const imgContainer = createCustomElement('div', 'cart__product__image-container'); // criei o meu carrinho
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', async (event) => {
    ...
    saveCartID(id);
    const createProduct = createCartProductElement(await fetchProduct(id));
    document.querySelector('.cart__products').appendChild(createProduct);
  });
